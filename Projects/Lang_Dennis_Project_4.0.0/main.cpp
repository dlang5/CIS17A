...
//get each side and then confirm();
    ...
    if(sel == 'm'){
        ...
        roy1 = titleM[rand()%10] + s1 + " the " + adjM[rand()%5];
    }
...
//verify 'y' or 'n' for replay  
    ...
    if(yOrN != 'n'){
        ...
        if (yOrN == 'y'){
            ...
            maxWin=retMax<int>(ptr[PLAYER1].getTots.winTot,
                    ptr[PLAYER2].getTots.winTot);
                        gmTot = ptr[PLAYER1].getTots.winTot 
                              + ptr[PLAYER2].getTots.winTot;
